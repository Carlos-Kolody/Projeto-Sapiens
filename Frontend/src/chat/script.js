...
async function sendMessage(event) {
  ...
  if (file) {
    userMessageContent += `<br><small><em>Arquivo anexado: ${file.name}</em></small>`;
  }
...
function saveMessageToConversation(content, sender) {
  ...
  if (
    APP_STATE.conversations[APP_STATE.currentConversationId].messages.length ===
    1
  ) {
    const title =
...
function updateConversationsList() {
  ...
  sortedConversations.forEach((conversation) => {
    ...
    conversationItem.className = `chat-history-item ${
      isActive ? "active" : ""
    }`;
    conversationItem.innerHTML = `<div class="chat-item-content">${escapeHtml(
      conversation.title
    )}</div><button class="delete-chat-btn" data-id="${
      conversation.id
...
function confirmClearAll() {
  ...
  DOM_ELEMENTS.modalMessage.textContent = `Tem certeza que deseja excluir todas as ${
    Object.keys(APP_STATE.conversations).length
